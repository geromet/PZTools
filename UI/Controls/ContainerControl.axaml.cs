...
public partial class ContainerControl : UserControl
{
    ...
    public void Load(Container c, UndoRedoStack undoRedo)
    {
        ...
        try
        {
            ...
            ContentGrid.ColumnDefinitions[2].Width = hasJunk ? GridLength.Auto : new GridLength(0);

            bool hasProc = c.ProcListEntries.Count > 0;
            ProcListPanel.IsVisible = hasProc;
            ContentGrid.ColumnDefinitions[3].Width = hasProc ? GridLength.Auto : new GridLength(0);
        }
        finally
        {
            _loading = false;
